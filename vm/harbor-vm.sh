...
# ==============================================================================
# Harbor VM - Creates a Harbor Registry Virtual Machine with Docker and Trivy
# ==============================================================================
...
function select_os() {
  if OS_CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "SELECT OS" --radiolist \
    "Choose Operating System for Harbor VM" 14 68 4 \
...
function default_settings() {
  ...
  echo -e "${CREATING}${BOLD}${DGN}Creating a Harbor VM using the above settings${CL}"
}
...
function advanced_settings() {
  ...
  if DISK_SIZE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Disk Size in GiB (e.g., 40, 80)" 8 58 "40" --title "DISK SIZE" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 harbor --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="harbor"
    else
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a Harbor VM?" --no-button Do-Over 10 58); then
    echo -e "${CREATING}${BOLD}${DGN}Creating a Harbor VM using the above advanced settings${CL}"
  else
    header_info
...
# ==============================================================================
# HARBOR INSTALLATION SCRIPTS
# ==============================================================================
function create_harbor_install_script() {
  local file="$1"

  virt-customize -q -a "${file}" --run-command "cat > /root/install-harbor.sh << 'INSTALLEOF'
#!/bin/bash
exec > /var/log/install-harbor.log 2>&1
echo \"[\$(date)] Starting Harbor installation on first boot\"

if [ -f /root/.harbor-installed ]; then
  echo \"[\$(date)] Harbor already installed, starting services\"
...
for i in {1..30}; do
  if docker info >/dev/null 2>&1; then
    echo \"[\$(date)] Docker is available\"
    break
  fi
  echo \"[\$(date)] Waiting for Docker... attempt \$i/30\"
  sleep 2
done

if ! docker info >/dev/null 2>&1; then
  echo \"[\$(date)] Docker not available, exiting\"
  exit 1
fi

# Get Harbor release
echo \"[\$(date)] Fetching latest Harbor release\"
RELEASE=\$(curl -fsSL https://api.github.com/repos/goharbor/harbor/releases/latest | grep '\"tag_name\":' | sed -E 's/.*\"([^\"]+)\".*/\1/')

if [ -z \"\$RELEASE\" ]; then
  echo \"[\$(date)] Failed to get Harbor release\"
  exit 1
fi

echo \"[\$(date)] Downloading Harbor \$RELEASE\"
cd /opt
curl -fsSL \"https://github.com/goharbor/harbor/releases/download/\${RELEASE}/harbor-online-installer-\${RELEASE}.tgz\" -o harbor-online-installer.tgz
tar -xzf harbor-online-installer.tgz
rm -f harbor-online-installer.tgz

echo \"[\$(date)] Configuring Harbor\"
cd /opt/harbor
...
echo \"[\$(date)] Installing Harbor with Trivy\"
./install.sh --with-trivy

echo \"\${RELEASE#v}\" > /root/.harbor
touch /root/.harbor-installed
echo \"[\$(date)] Harbor installation completed successfully\"
INSTALLEOF" >/dev/null

  virt-customize -q -a "${file}" --run-command "chmod +x /root/install-harbor.sh" >/dev/null
}

function create_harbor_service() {
  local file="$1"

  virt-customize -q -a "${file}" --run-command 'cat > /etc/systemd/system/install-harbor.service << "SERVICEEOF"
[Unit]
Description=Install Harbor on First Boot
After=docker.service install-docker.service
Requires=docker.service
ConditionPathExists=!/root/.harbor-installed

[Service]
Type=oneshot
ExecStart=/root/install-harbor.sh
RemainAfterExit=yes
...
' >/dev/null
  ...
  virt-customize -q -a "${file}" --run-command "systemctl enable install-harbor.service" >/dev/null
}
...
apt-get install -y qemu-guest-agent curl ca-certificates jq 2>/dev/null || true
...
if whiptail --backtitle "Proxmox VE Helper Scripts" --title "Harbor VM" --yesno "This will create a New Harbor Registry VM.\n\nIncludes:\n- Docker\n- Harbor Registry\n- Trivy Vulnerability Scanner\n\nProceed?" 14 58; then
  :
else
...
# IMAGE CUSTOMIZATION
# ==============================================================================
echo -e "${INFO}${BOLD}${GN}Preparing ${OS_DISPLAY} Qcow2 Disk Image for Harbor${CL}"

export LIBGUESTFS_BACKEND_SETTINGS=dns=8.8.8.8,1.1.1.1

create_docker_install_script "${FILE}"
create_docker_service "${FILE}"
create_harbor_install_script "${FILE}"
create_harbor_service "${FILE}"

DOCKER_INSTALLED_ON_FIRST_BOOT="yes"

msg_info "Installing base packages (qemu-guest-agent, curl, ca-certificates, jq)"
if virt-customize -a "${FILE}" --install qemu-guest-agent,curl,ca-certificates,jq >/dev/null 2>&1; then
...
# VM CREATION
# ==============================================================================
msg_info "Creating Harbor VM"

qm create $VMID -agent 1${MACHINE} -tablet 0 -localtime 1 -bios ovmf${CPU_TYPE} -cores $CORE_COUNT -memory $RAM_SIZE \
  -name $HN -tags community-script,harbor,registry -net0 virtio,bridge=$BRG,macaddr=$MAC$VLAN$MTU -onboot 1 -ostype l26 -scsihw virtio-scsi-pci
...
msg_ok "Created Harbor VM ${CL}${BL}(${HN})${CL}"
...
if [ "$START_VM" == "yes" ]; then
  msg_info "Starting Harbor VM"
  qm start $VMID >/dev/null 2>&1
  msg_ok "Started Harbor VM"
fi
...
echo -e "\n${INFO}${BOLD}${GN}Harbor VM Configuration Summary:${CL}"
...
if [ "$DOCKER_INSTALLED_ON_FIRST_BOOT" = "yes" ]; then
  ...
  echo -e "${TAB}${YW}Wait 5-10 minutes after boot for Harbor installation to complete${CL}"
else
  echo -e "${TAB}${DGN}Docker: ${BGN}Latest (via get.docker.com)${CL}"
  echo -e "${TAB}${YW}Harbor will be installed on first boot (5-10 minutes)${CL}"
fi

echo -e "${TAB}${DGN}Harbor: ${BGN}Latest with Trivy vulnerability scanner${CL}"

if [ -n "$VM_IP" ]; then
  echo -e "${TAB}${DGN}Harbor UI: ${BGN}http://${VM_IP}${CL}"
  echo -e "${TAB}${DGN}Default Login: ${BGN}admin / Harbor12345${CL}"
else
  echo -e "${TAB}${DGN}Harbor UI: ${BGN}http://<VM-IP>${CL}"
  echo -e "${TAB}${DGN}Default Login: ${BGN}admin / Harbor12345${CL}"
  echo -e "${TAB}${YW}Get IP: ${BL}qm guest cmd ${VMID} network-get-interfaces${CL}"
fi

echo -e "${TAB}${YW}Check installation progress: ${BL}cat /var/log/install-harbor.log${CL}"
